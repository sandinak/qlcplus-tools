...
RGB_ALTERNATES = {"Cyan": "Light Blue"}
...
COLOR_WHEEL_CHANNELS = ["Colour", "Color", "Color Wheel", "Color wheel"]

MOVEMENT_CHANNEL_NAMES = ["Pan", "Level", "Horizontal", "Tilt", "Vertical"]
...
""".strip()
 ...
 # this is a hack because XML doesn't reload. 
step_numbers={}
...
class QLC:
    ...
    def generate_color_scenes(self, fg):
        ...
        for color_name, color_data in rgbw.items():
            ...
            for head in fg.heads:
                ...
                mode_channels = definition.modes.find_by_name(
                    fixture.mode
                ).mode_channels
                ...
                if "Red" in mode_channels.names:
                    ...
                    for mc in mode_channels:
                        mc_num = mc.number
                        mc_name = mc.name
                        if (mc_name == "Intensity" or mc_name == "Dimmer" or 
                            mc_name == 'Master Dimmer' ):
                ...
                elif color_channels := intersection(
                    mode_channels.names, COLOR_WHEEL_CHANNELS
                ):
            ...
            if len(fg.heads.items) == len(fvals):
                sid = self.function(Type=Type, Path=Path, Name=Name, FixtureVals=fvals)

                if len(modes) == 1:
                    # only add once to collection
                    self.function(Type="Collection",
                                Path='Colors',
                                Name=color_name,
                                SceneIds=[sid])
                    self.workspace.sync()
    ...
    def generate_capability_scenes(self, fg):
        ...
        NOTE: this one does NOT work across heads.. yet so is only gonna work 
        when the heads are all the same."""
        ...
        for mode_channel in mode.mode_channels:
            ...
            for capability in channel.capabilities:
                ...
                # generate scene
                sid = self.function(Type="Scene", Path=Path, Name=Name, FixtureVals=fvals)
        ...
        if fixture_definition.type == "Moving Head":
            ...
            for name, pos in BASE_POSITIONS.items():
                h, v = pos
                Name = "/".join([Path, name])
                print(f" generating base position for {Name}")
                fvals = {}
                for head in fg.heads:
                    fval = []
                    for mc in mode.mode_channels:
                        channel = fixture_definition.channels.find_by_name(mc.name)
                        print(f"   {mc.name} {channel.group}")
                        # no consistency among fixtures here.. frustrating.
                        if (
                            channel.group == "Pan" or "Pan" in channel.name
                        ) and "Speed" not in channel.name:
                            fval.append(f"{mc.number},{h}")
                        elif (
                            channel.group == "Tilt" or "Tilt" in channel.name
                        ) and "Speed" not in channel.name:
                ...
                # add to grouping collection
                cid = self.function(Type='Collection', 
                                    Path='Positions',
                                    Name=name,
                                    SceneIds= [sid]
                                    )
    ...
    def function(self, **kwargs):
        ...
        if not func and Name:
            func = functions.find_by_type_and_name(Type, Name)
        ...
        if Type == "Script":
            ...
            self.f_collection(this_func, **kwargs)
        return ID

    def f_collection(self, f, **kwargs):
        ''' take a list of SceneIDs and add to collection if not 
            already there'''
        global step_numbers
        scene_ids = kwargs.get('SceneIds')
        # check all the existing steps 
        f_id = f.get("ID")
        if f_id not in step_numbers:
            step_numbers[f_id] = -1
        found_scene_ids = []
        steps = f.findall("{%s}Step" % Workspace.xmlns)
        for step in steps:
            s_number = int(step.get("Number"))
            if s_number and s_number > step_numbers[f_id]:
                    step_numbers[f_id] = s_number

            sid = step.text
            if sid in scene_ids:
                found_scene_ids.append(sid)

        # add the step for this scene if it's not there
        for sid in scene_ids:
            if not sid in found_scene_ids:
                step_numbers[f_id] += 1
                this_step = ET.SubElement(f,"Step")
                this_step.set("Number", str(step_numbers[f_id]))
                this_step.text = str(sid)
    ...
    def fattr_speed(self, f, **kwargs):
        default_speed = {"FadeIn": "0", "FadeOout": "0", "Duration": "0"}
...
class Workspace(QLC):
    ...
    def sync(self):
        '''resync the data'''
        tmp = ET.tostring(self.root)
        self.root=ET.fromstring(tmp)
        self.creator = Creator(self.root)
        self.engine = Engine(self.root)
...
class Universes(InputOutputMap):
    ...
    def find_by_name(self, name, type=None):
        for i in self.items:
            if i.name == name:
                return i
...
class Functions(Engine):
    ...
    def find_by_type_and_name(self, ftype, name):
        ''' multiple types of functions'''
        for i in self.items:
            if i.type == ftype and i.name == name:
                return i 
...
class FixtureDefinition(FixtureDefinitions):
    ...
    def __init__(self, path):
        ...
        # set accessors
        self.manufacturer = self.root.find(
            "{%s}Manufacturer" % FixtureDefinition.xmlns
        ).text
