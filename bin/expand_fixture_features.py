...
def get_args():
    ...
    parser.add_argument(
        "-i", "--inplace", help="Overwrite the showfile in place.", action="store_true"
    )
    parser.add_argument(
        "-d", "--dump", help="Dump modifications to STDOUT", action="store_true"
    )
    parser.add_argument(
        "-o", "--outputfile", type=str, default=None, help="Write to output file"
    )
    parser.add_argument(
        "-F", "--overwrite", action="store_true", help="overwrite existing output file"
    )
    parser.add_argument("showfile", type=str, help="Showfile to extend")
